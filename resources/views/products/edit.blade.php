        ...
        <div class="col-xl-12 order-xl-1">
            <div class="card bg-secondary shadow">
                ...
                <div class="card-body">
                    ...
                    <form method="post" action="{{ route('products.update',$product) }}" enctype="multipart/form-data" autocomplete="off">
                        @csrf
                        @method('PATCH')
                        {{-- <h5>hello</h5> --}}
                        ...
                        <div class="pl-lg-4">
                            ...
                            <category-form hasError="{{$errors->has('category')}}" adminid="{{auth()->user()->admin->id}}" errormessage="{{$errors->first('category')}}" :oldcategory="{{$product->type}}" oldtype="{{$product->type}}"></category-form>
                            ...
                            <div class="form-group @error('new') has-danger @enderror">
                                ...
                                <div class="custom-control custom-checkbox">
                                    <input type="checkbox" class="custom-control-input" name="new" id="new-input" @if($product->new) checked @endif>
                            ...
                            <div class="form-group @error('colors[]') has-danger @enderror">
                                ...
                                <div class="custom-control custom-checkbox">
                                    <input type="checkbox" @if ($product->colors->contains($item))
                                    checked
                                    @endif class="custom-control-input"  name="colors[]" id="color{{$item->id}}" value="{{$item->id}}">
                            ...
                            <div class="form-group @error('note') has-danger @enderror">
                                ...
                                <input type="text" name="note" id="input-note" value="{{$product->note}}" class="form-control form-control-alternative text-dark @error('note') is-invalid @enderror" placeholder="{{ __('Note') }}" value="{{ old('note') ?? $product->note }}" r autofocus>
                            ...
                            <div class="form-group @error('description') has-danger @enderror">
                                ...
                                <textarea class="form-control form-control-alternative @error('description') is-invalid @enderror text-dark"  rows="3" placeholder="Write a large text here ..." name="description">{{ old('description')?? strip_tags($product->description) }}</textarea>
                                @error('description')
                                <span class="invalid-feedback d-block" role="alert">
